...
{
public:
    AnalyserComponent()    // constructor
        : forwardFFT (fftOrder),
          window (fftSize, dsp::WindowingFunction<float>::hann)
    {
        setOpaque (true);
        setAudioChannels (2, 0);  // we want a couple of input channels but no outputs
        startTimerHz (30); // muestras de video 24 frames per sec
        setSize (700, 500);
    }
    ...
    void timerCallback() override
    {
        ...
        {
            drawNextFrameOfSpectrum(); // dibuja
            nextFFTBlockReady = false; // borra
            repaint();
        }
    }
    ...
    void drawNextFrameOfSpectrum()
    {
        ...
        window.multiplyWithWindowingTable (fftData, fftSize); // hacer ventaneo, se manejan 2 arreglos pq no puedo tocar el buffer

        // then render our FFT data..
        forwardFFT.performFrequencyOnlyForwardTransform (fftData); // la fft me da un numero complejo y yo tengo un arreglo real docs.juce.com/master/classdsp_1_1FFT.html, perform acepta complejos
...
private:
    ...
    float fifo [fftSize];
    float fftData [2 * fftSize]; // es el doble del buffer
    int fifoIndex = 0;
